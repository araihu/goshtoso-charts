...
func TestFunnelRendersConfiguredChart(t *testing.T) {
	...
	instance := Funnel(FunnelConfig{
		...
		Series: []FunnelSeries{{
			...
			Options: []charts.SeriesOpts{charts.WithAnimationOpts(opts.Animation{Animation: opts.Bool(false)})},
		}},
		Width: "720px", Height: "360px",
		GlobalOptions: []charts.GlobalOpts{
			charts.WithTitleOpts(opts.Title{Title: "Conversion"}),
			charts.WithColorsOpts(opts.Colors{"#should-not-win"}),
		},
		SeriesOptions: []charts.SeriesOpts{charts.WithLabelOpts(opts.Label{Show: opts.Bool(true), Position: "left"})},
		Style:         charttheme.Style{Palette: charttheme.PaletteAraiHu, Colors: []string{"#123456"}, Class: "min-h-80"},
	})

	if instance.Kind() != chartcomponents.KindEChartsFunnel {
	...
	if strings.Contains(markup, "#should-not-win") {
		t.Error("explicit Style.Colors did not override global color option")
	}
}
