...
func HeatMap(cfg HeatMapConfig) Instance {
	if err := validateHeatMapConfig(cfg); err != nil {
		return newInvalidInstance(chartcomponents.KindEChartsHeatMap, err)
	}
	...
	for _, series := range cfg.Series {
		...
		options := make([]charts.SeriesOpts, 0, len(cfg.SeriesOptions)+len(series.Options)+1)
	...
	return newInstance(chartcomponents.KindEChartsHeatMap, Config{
...
func heatMapGlobalOptions(cfg HeatMapConfig) []charts.GlobalOpts {
	...
	options = append(options, cfg.GlobalOptions...)
