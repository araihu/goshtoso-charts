...
func TestHeatMapRendersCartesianChart(t *testing.T) {
	...
	instance := HeatMap(HeatMapConfig{
		...
		GlobalOptions: []charts.GlobalOpts{
			charts.WithTitleOpts(opts.Title{Title: "Traffic density"}),
			charts.WithColorsOpts(opts.Colors{"#should-not-win"}),
			charts.WithVisualMapOpts(opts.VisualMap{Min: -1, Max: 99, InRange: &opts.VisualMapInRange{Color: []string{"#should-not-win"}}}),
		},
		SeriesOptions: []charts.SeriesOpts{charts.WithAnimationOpts(opts.Animation{Animation: opts.Bool(false)})},
		Style:         charttheme.Style{Palette: charttheme.PaletteAraiHu, Colors: []string{"#123456"}, Class: "min-h-80"},
	})

	if instance.Kind() != chartcomponents.KindEChartsHeatMap {
	...
	if strings.Contains(markup, "#should-not-win") || strings.Contains(markup, `"max":99`) {
		t.Error("explicit Style.Colors and typed range did not override global visual options")
	}
}
...
func TestHeatMapRendersCalendarChart(t *testing.T) {
	...
	instance := HeatMap(HeatMapConfig{
		...
		Calendar: &HeatMapCalendar{
			Start: start, End: end,
			Options: opts.Calendar{Top: "80", CellSize: "20", Range: []string{"ignored"}},
		},
	...
	if strings.Contains(markup, `"ignored"`) {
		t.Error("typed calendar range did not override Calendar.Options.Range")
	}
}
