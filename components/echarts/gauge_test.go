...
func TestGaugeRendersStandardVariantWithDefaults(t *testing.T) {
	...
	instance := Gauge(GaugeConfig{
		...
		GlobalOptions: []charts.GlobalOpts{
			charts.WithTitleOpts(opts.Title{Title: "Delivery"}),
	...
	if instance.Kind() != chartcomponents.KindEChartsGauge {
...
func TestGaugeRendersProgressVariantWithSameKind(t *testing.T) {
	...
	instance := Gauge(GaugeConfig{
		...
		Series: []GaugeSeries{{
			...
			Options: []charts.SeriesOpts{func(series *charts.SingleSeries) {
				series.Progress.Width = 18
			}},
		}},
	})
	if instance.Kind() != chartcomponents.KindEChartsGauge {
