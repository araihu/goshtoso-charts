...
func TestPieRejectsInvalidDataContract(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"negative series shadow": {PieConfig{Label: "States", Series: []PieSeries{{Name: "States", Data: validSeries.Data, Options: SeriesOptions{Emphasis: &EmphasisOptions{ItemStyle: &ItemStyle{ShadowBlur: -1}}}}}}, `pie chart item shadow blur must be nonnegative`},
		"negative data shadow":   {PieConfig{Label: "States", Series: []PieSeries{{Name: "States", Data: []PieData{{Name: "Open", Value: 1, ItemStyle: &ItemStyle{ShadowBlur: -1}}}}}}, `pie chart item shadow blur must be nonnegative`},
