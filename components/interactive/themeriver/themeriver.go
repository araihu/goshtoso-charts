// Package themeriver provides the canonical interactive temporal stream graph API.
//
// ThemeRiver-specific types and implementation live here; shared
...
// Instance is the renderer-neutral chart instance returned by ThemeRiver.
type Instance = chart.Instance

// Config describes an accessible temporal stream graph.
//
...
// Stream is one named temporal layer.
//
...
// Point is one typed value at an instant.
type Point struct {
	Time  time.Time
	Value float64
}

// Layout positions the temporal axis inside the chart.
type Layout struct {
	LeftPercent   *float64
...
// BoundaryGap reserves percentage space before and after the river.
...
func ThemeRiver(cfg Config) Instance {
	if err := validateConfig(cfg); err != nil {
		return internalinteractive.Invalid(chartcomponents.KindInteractiveThemeRiver, err)
	}

	style := themeRiverStyle(cfg)
	themeRiverChart := charts.NewThemeRiver()
	...
	if cfg.Layout.LeftPercent != nil {
		axis.Left = internalinteractive.Percentage(*cfg.Layout.LeftPercent)
	}
	if cfg.Layout.RightPercent != nil {
		axis.Right = internalinteractive.Percentage(*cfg.Layout.RightPercent)
	}
	if cfg.Layout.TopPercent != nil {
		axis.Top = internalinteractive.Percentage(*cfg.Layout.TopPercent)
	}
	if cfg.Layout.BottomPercent != nil {
		axis.Bottom = internalinteractive.Percentage(*cfg.Layout.BottomPercent)
	} else {
		axis.Bottom = "10%"
	}
	globalOptions = append(globalOptions, charts.WithSingleAxisOpts(axis))
	themeRiverChart.SetGlobalOptions(globalOptions...)
	...
	themeRiverChart.AddSeries(cfg.Label, data, seriesOptions...)

	replacements := []internalinteractive.ScriptReplacement{}
	if cfg.BoundaryGap.StartPercent != nil {
		gap, _ := json.Marshal([]string{internalinteractive.Percentage(*cfg.BoundaryGap.StartPercent), internalinteractive.Percentage(*cfg.BoundaryGap.EndPercent)})
		replacements = append(replacements, internalinteractive.ScriptReplacement{
			Old: `"type":"themeRiver"`,
			New: `"type":"themeRiver","boundaryGap":` + string(gap),
		})
	}
	return internalinteractive.New(chartcomponents.KindInteractiveThemeRiver, internalinteractive.RenderConfig{
		Label: cfg.Label, Caption: cfg.Caption, Chart: themeRiverChart, Style: style, ResponsiveWidth: internalinteractive.ResponsiveWidth(cfg.Width),
...
func themeRiverStyle(cfg Config) charttheme.Style {
	style := cfg.Style
...
func validateConfig(cfg Config) error {
	...
	for name, value := range map[string]*float64{
		...
		if value != nil && !internalinteractive.ValidPercentage(*value) {
	...
	if cfg.BoundaryGap.StartPercent != nil &&
		(!internalinteractive.ValidPercentage(*cfg.BoundaryGap.StartPercent) || !internalinteractive.ValidPercentage(*cfg.BoundaryGap.EndPercent)) {
...
func themeRiverDetailRows(streams []Stream, limit int) themeRiverValueRows {
	total := 0
