...
test("four variants preserve source data, index swap, palette, grid, rotation, shading, and local dependency order", async () => {
  ...
  try {
    ...
    for (const asset of ["/charts/assets/js/runtime/three-d/2.0.9/runtime.min.js", "/charts/assets/js/runtime/echarts/5.6.0/echarts.min.js"]) {
...
test("large centered modal preserves instance and opaque direct PNG", async () => {
  ...
  try {
    ...
    await wrapper.getByRole("button", { name: "Export basic bar3d example" }).click();
