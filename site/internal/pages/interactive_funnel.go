...
func fixedInteractiveFunnelData(callIndex int) []interactive.FunnelData {
	if callIndex < 0 {
		panic("interactive Funnel call index must be nonnegative")
	}
	rng := rand.New(rand.NewSource(interactiveFunnelSeed))
	var data []interactive.FunnelData
	for call := 0; call <= callIndex; call++ {
		data = make([]interactive.FunnelData, len(interactiveFunnelDimensions))
		for index, name := range interactiveFunnelDimensions {
			data[index] = interactive.FunnelData{Name: name, Value: float64(rng.Intn(50))}
		}
	}
	return data
}

func interactiveFunnelOptions(title, filename string) interactive.ChartOptions {
	options := controlledOptions(title, filename)
	options.Legend = &interactive.LegendOptions{Show: interactive.Bool(true), Left: "center", Bottom: "0"}
...
func sampleInteractiveFunnelLabels() interactive.FunnelConfig {
	return interactive.FunnelConfig{
		...
		SeriesOptions: interactive.SeriesOptions{Label: &interactive.LabelOptions{Show: interactive.Bool(true), Position: "left"}},
		Style:         charttheme.Style{Class: "max-w-5xl mx-auto"},
	}
}
...
func interactiveFunnelBaseCode() string {
  ...
  Options: interactive.ChartOptions{
    Title: &interactive.TitleOptions{Text: "basic funnel example"},
  },
})`
}
...
func interactiveFunnelLabelsCode() string {
  ...
  SeriesOptions: interactive.SeriesOptions{
    Label: &interactive.LabelOptions{
