...
func interactiveScatterData(values [6]float64, symbols bool) []interactive.ScatterData {
	data := make([]interactive.ScatterData, len(values))
	for index, value := range values {
		data[index] = interactive.ScatterData{Value: value}
		if symbols {
...
func interactiveScatterOptions(title, filename string) interactive.ChartOptions {
...
func sampleInteractiveScatterLabels() interactive.ScatterConfig {
	config := interactive.ScatterConfig{
		...
		SeriesOptions: interactive.SeriesOptions{Label: &interactive.LabelOptions{Show: interactive.Bool(true), Position: "right"}},
		Style:         charttheme.Style{Class: "max-w-5xl mx-auto"},
	}
	return config
}

func sampleInteractiveScatterSplitLines() interactive.ScatterConfig {
	options := interactiveScatterOptions("splitline options", "split-line-sports-scatter")
	options.XAxis = &interactive.AxisOptions{Name: "Sports", ShowSplitLine: interactive.Bool(true)}
	options.YAxis = &interactive.AxisOptions{Name: "Score", ShowSplitLine: interactive.Bool(true)}
	return interactive.ScatterConfig{
...
func sampleInteractiveEffectScatter() interactive.ScatterConfig {
	return interactive.ScatterConfig{
		Label: "Basic dunk effect scatter", Caption: "A ripple emphasizes each player without changing the six exact dunk values.", Variant: interactive.ScatterVariantEffect,
		XAxis: interactiveScatterPlayers, Series: []interactive.ScatterSeries{{Name: "Dunk", Data: interactiveScatterData([6]float64{37, 31, 85, 26, 13, 90}, false)}},
		Width: "100%", Height: "420px", Options: interactiveScatterOptions("basic EffectScatter example", "basic-dunk-effect-scatter"),
		Style: charttheme.Style{Class: "max-w-5xl mx-auto"},
	}
}

func sampleInteractiveEffectScatterStyles() interactive.ScatterConfig {
	return interactive.ScatterConfig{
		Label: "Dunk and shoot ripple styles", Caption: "Dunk uses a slower, larger stroke ripple; Shoot uses a faster, smaller fill ripple.", Variant: interactive.ScatterVariantEffect,
		XAxis: interactiveScatterPlayers,
		Series: []interactive.ScatterSeries{
			{Name: "Dunk", Data: interactiveScatterData([6]float64{94, 63, 33, 47, 78, 24}, false), Ripple: &interactive.RippleOptions{Period: 4, Scale: 10, BrushType: "stroke"}},
			{Name: "Shoot", Data: interactiveScatterData([6]float64{59, 53, 57, 21, 89, 99}, false), Ripple: &interactive.RippleOptions{Period: 3, Scale: 6, BrushType: "fill"}},
		},
...
func interactiveChartScatterCode() string {
  ...
  Series: []interactive.ScatterSeries{{
    Name: "Category A",
    Data: []interactive.ScatterData{{Value: 81, Symbol: "roundRect", SymbolSize: 20, SymbolRotate: 10}},
  }},
  Width: "100%",
  Height: "420px",
})`
