// Package araihuassets updates Goshtoso Charts' allowlisted embedded brand
// fallbacks from an already extracted, immutable araihu/assets release. It
// performs no network access; callers own release download and verification.
...
// Mapping names one release file, its repository destination, and the exact
// catalog identity and semantic roles that must select it.
type Mapping struct {
...
// ReleaseIdentity replaces only immutable release identity. Repository-owned
// mappings cannot be changed by dispatch payloads.
...
// identity. Update has no network client.
type Options struct {
	RepoRoot      string
	ReleaseRoot   string
	ManifestPath  string
...
// ApplyError reports a failed transaction and rollback state.
...
// Update verifies pinned release metadata, catalog selection, file checksums,
// and path confinement before transactionally replacing changed files.
...
func loadManifest(repo *os.Root, manifestPath string, identity *ReleaseIdentity) (Manifest, []byte, error) {
	...
	if err := decodeExactJSON(manifestBytes, "manifest", &manifest); err != nil {
...
func validateManifest(manifest Manifest) error {
	...
	for index, mapping := range manifest.Mappings {
		...
		for _, role := range []string{
			mapping.CanonicalName, mapping.Namespace, mapping.Product, mapping.Artwork,
			mapping.Appearance, mapping.Surface, mapping.Framing, mapping.Format,
		} {
			if role == "" {
...
func loadReleaseDocument(root *os.Root, manifest Manifest) (releaseDocument, error) {
	...
	if err := decodeExactJSON(releaseBytes, "release.json", &document); err != nil {
	...
	if document.IdentityRevision != 11 {
		return releaseDocument{}, fmt.Errorf("release.json identityRevision = %d, want 11", document.IdentityRevision)
	}
	if !sha256RE.MatchString(document.CatalogSHA256) {
		return releaseDocument{}, fmt.Errorf("release.json catalogSha256 = %q", document.CatalogSHA256)
	}
	return document, nil
}
...
func buildInventory(document releaseDocument, mappings []Mapping) (map[string]releaseFile, error) {
	...
	foldedPaths := make(map[string]string, len(document.Files))
	for index, entry := range document.Files {
		...
		if previous, exists := foldedPaths[folded]; exists {
			return nil, fmt.Errorf("release.json file collision between %q and %q", previous, entry.Path)
		}
...
func loadReleaseCatalog(root *os.Root, document releaseDocument, inventory map[string]releaseFile, release string) (catalogDocument, error) {
	...
	if int64(len(catalogBytes)) != catalogEntry.Size {
		return catalogDocument{}, fmt.Errorf("catalog.json size = %d, want %d", len(catalogBytes), catalogEntry.Size)
	}
	...
	if err := decodeExactJSON(catalogBytes, "catalog.json", &catalog); err != nil {
		return catalogDocument{}, err
	}
	if catalog.SchemaVersion != 1 {
		return catalogDocument{}, fmt.Errorf("catalog.json schemaVersion = %d, want 1", catalog.SchemaVersion)
	}
	if catalog.Release != release {
		return catalogDocument{}, fmt.Errorf("catalog release = %q, want %q", catalog.Release, release)
	}
	if catalog.IdentityRevision != 11 {
		return catalogDocument{}, fmt.Errorf("catalog identityRevision = %d, want 11", catalog.IdentityRevision)
	}
	names := make(map[string]struct{}, len(catalog.Assets))
	paths := make(map[string]string, len(catalog.Assets))
	for index, asset := range catalog.Assets {
		if asset.CanonicalName == "" || !sha256RE.MatchString(asset.SHA256) {
			return catalogDocument{}, fmt.Errorf("catalog asset %d has invalid identity", index)
		}
		foldedName := strings.ToLower(asset.CanonicalName)
		...
		if err := validateRelativePath(asset.Path, "catalog path"); err != nil {
			return catalogDocument{}, fmt.Errorf("catalog asset %d: %w", index, err)
		}
		foldedPath := strings.ToLower(asset.Path)
		if previous, exists := paths[foldedPath]; exists {
			return catalogDocument{}, fmt.Errorf("catalog path collision between %q and %q", previous, asset.Path)
		}
		paths[foldedPath] = asset.Path
	}
	return catalog, nil
}
...
func decodeExactJSON(contents []byte, name string, destination any) error {
		...
		case '{':
			...
			for decoder.More() {
				...
				if previous, exists := seen[folded]; exists {
					return fmt.Errorf("JSON key collision between %q and %q", previous, key)
				}
				seen[folded] = key
