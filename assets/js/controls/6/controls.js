(function () {
  ...
  function actionTrigger(action) {
    ...
    return (dropdown && dropdown.querySelector(":scope > button, :scope > [data-popover-trigger] > button")) || action;
  }
  ...
  function setFullscreenState(wrapper, active) {
    ...
    wrapper.querySelectorAll('[id$="-stacked"], button[id$="-fullscreen-action"]:not([role="menuitem"])').forEach(function (entry) {
      entry.hidden = active;
    });
    var collapse = wrapper.querySelector('[data-goshtoso-chart-control="collapse"]');
    if (collapse) {
  ...
  function restoreFullscreenState(wrapper, restoreFocus) {
    if (!wrapper) return;
  ...
  function leaveFallback(restoreFocus) {
    ...
    restoreFullscreenState(wrapper, restoreFocus);
  }
  ...
  function collapseFullscreen(button) {
    var wrapper = wrapperFor(button);
    if (!wrapper || wrapperMode(wrapper) !== "enabled") return;
    var collapse = wrapper.querySelector('[data-goshtoso-chart-control="collapse"]');
    if (!collapse || collapse.hidden) return;
    var fullscreenWrapper = document.fullscreenElement && wrapperFor(document.fullscreenElement);
    if (fullscreenWrapper === wrapper && typeof document.exitFullscreen === "function") {
      var exit = document.exitFullscreen();
      if (exit && typeof exit.then === "function") {
        exit.then(function () {
          if (document.fullscreenElement !== wrapper) restoreFullscreenState(wrapper, true);
        }, function () { restoreFullscreenState(wrapper, true); });
      } else {
        restoreFullscreenState(wrapper, true);
      }
      return;
    }
    if (fallbackWrapper === wrapper) leaveFallback(true);
    else restoreFullscreenState(wrapper, true);
  }
  ...
  function prepareActions(wrapper) {
    ...
    var stackedTrigger = wrapper && wrapper.querySelector('[id$="-stacked"] > button, [id$="-stacked"] > [data-popover-root] > [data-popover-trigger] > button');
    if (stackedTrigger) stackedTrigger.setAttribute("aria-label", "Expand " + chartLabel(wrapper));
    var exportTrigger = wrapper && wrapper.querySelector('[id$="-export"] > button, [id$="-export"] > [data-popover-root] > [data-popover-trigger] > button');
  ...
  async function copyChartAsPNG(button) {
    ...
    try {
      ...
      var capability = wrapper.dataset.goshtosoChartCapability;
      var blob;
      if (capability === "static-svg") blob = await staticPNGBlob(wrapper);
      else if (capability === "interactive-raster") blob = await interactivePNGBlob(wrapper);
      else throw new Error("Unsupported chart copy.");
  ...
  document.addEventListener("keydown", function (event) {
    ...
    if (event.target && typeof event.target.closest === "function") {
      ...
      var trigger = menu && menu.parentElement && menu.parentElement.querySelector(":scope > button, :scope > [data-popover-trigger] > button");
      if (trigger) setTimeout(function () { trigger.focus(); }, 50);
    }
    if (fallbackWrapper) leaveFallback(true);
    var wrapper = document.fullscreenElement && wrapperFor(document.fullscreenElement);
    if (wrapper && document.exitFullscreen) {
  ...
  window.__goshtosoChartsControls = {
    ...
    copyFromMenu: function (element) { copyChartAsPNG(element); },
