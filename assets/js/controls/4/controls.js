(function () {
  ...
  var wrapperModes = { enabled: true, disabled: true, hidden: true };
  ...
  function wrapperMode(wrapper) {
    ...
    return wrapperModes[mode] ? mode : "enabled";
  }
  ...
  function applyWrapperMode(wrapper, mode, focusReturn, emitChange) {
    if (!wrapper || !wrapperModes[mode]) return false;
    ...
    if ((mode === "hidden" || (mode === "disabled" && active && actionFieldset(wrapper) && actionFieldset(wrapper).contains(active))) &&
      ...
      if (focusReturn instanceof HTMLElement && focusReturn.isConnected && !wrapper.contains(focusReturn)) {
        focusReturn.focus();
  ...
  document.addEventListener("goshtoso-charts:set-wrapper-mode", function (event) {
    ...
    if (!wrapper || !wrapperModes[detail.mode]) return;
